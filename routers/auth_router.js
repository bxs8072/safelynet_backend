...
router.post('/register', registerVerifier, [
    ...
    async (req, res, next) => {
        ...
        if (!errors.isEmpty()) {
            return res.status(422).json({ error: errors.array()[0].msg, param: errors.array()[0].param });
        }
        ...
        try {
            ...
            return res.json({
                message: "Click on link to activate your account",
                link: activationLink
            })
...
router.post('/login', loginVerifier, async (req, res, next) => {
    ...
    if (!passwordMatched) {
        return res.json({ message: "Invalid Email or Password. Try again" })
    } 
